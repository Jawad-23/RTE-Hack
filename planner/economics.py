...
def evaluate(setup: str, crop: str, area_m2: float, growing_months: float, cooling_kwh_year: float, solar_kw: float) -> dict:
    ...
    Assumes the solar panels cover the cooling electricity, so cooling_kwh_year adds no running cost.
    ...
    capex = area_m2 * s["capex_qar_m2"] + solar_kw * _settings()["solar_capex_qar_kw"]
    revenue = area_m2 * c["yield_kg_m2_year"] * p["price_qar_kg"] * (growing_months / 12) * s["yield_factor"]
    opex = area_m2 * s["opex_qar_m2_year"]
    profit = revenue - opex
    return {
        ...
        "water_l_day": round(float(area_m2 * (c["water_l_m2_day"] + s["water_l_m2_day_extra"])), 2),
    }
