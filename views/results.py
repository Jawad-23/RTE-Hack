...
# ---------- header ----------
country = ((plan.get("assumptions") or {}).get("price_country") or {}).get("name")
place = state.site_name(lat, lon, lang) or (t("site_in", lang).format(country=country) if country else t("p_pinned", lang))
h1, h2 = st.columns([1.4, 1.6], vertical_alignment="bottom")
h1.markdown(
    f'<div class="cr-eyebrow">{t("results_for", lang)}</div>'
    f'<div class="cr-place">{place}</div>'
    f'<div class="cr-mono">{state.coords(lat, lon)}</div>',
    unsafe_allow_html=True,
)
with h2, st.container(key="res_actions"):
    b1, b2, b3 = st.columns([1, 0.8, 1.35])
    b1.download_button(t("download_plan", lang), icon=":material/download:", data=json.dumps(plan, ensure_ascii=False, indent=2),
                       file_name=f"croptions-plan-{lat:.2f}-{lon:.2f}.json", mime="application/json", use_container_width=True)
    with b2.popover(t("share", lang), icon=":material/share:", use_container_width=True):
...
if rec:
    ...
    with st.container(key="hero"):
        ...
        if c1.button(t("ask_why", lang), type="primary", use_container_width=True):
            ...
            st.rerun()
        c2.markdown(f'<a href="#plan-details" style="display:inline-block;border:1px solid rgba(255,253,248,.35);color:#FFFDF8;'
...
# ---------- the full plan, organised in tabs ----------
st.markdown('<div id="plan-details"></div>', unsafe_allow_html=True)
tab_money, tab_site, tab_crops, tab_energy, tab_details = st.tabs(
    [t("tab_money", lang), t("tab_site", lang), t("tab_crops", lang), t("tab_energy", lang), t("tab_details", lang)])
with tab_money:
